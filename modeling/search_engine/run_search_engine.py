...
def create_wiki_index():
    ...
    wiki_data_path = os.path.join(PERSISTENT_DIR, 'data/wiki/enwiki-latest-abstract.xml.gz')
    ...
    if not os.path.exists(join_module_path('cache/index.json')) or not os.path.exists(join_module_path('cache/documents.json')):
        ...
        with open(join_module_path(f'cache/index.json'), 'w') as f:
        ...
        with open(join_module_path(f'cache/documents.json'), 'w') as f:
