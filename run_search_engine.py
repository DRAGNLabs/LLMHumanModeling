...
def do_all():
    ...
    if not os.path.exists('./search_engine/cache/search_index.json') or not os.path.exists('./search_engine/cache/documents.json'):
        
        if not os.path.exists('./search_engine/cache'):  # Make a 'cache' directory
            os.mkdir('./search_engine/cache')

        # Store search_index as .json 
        with open(f'./search_engine/cache/search_index.json', 'w') as f:
    ...
    abs_2_search = Index.get_rand_doc()
